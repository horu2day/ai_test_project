...
def translate_extracted(extracted, tokenizer):
    ...
    translate = GoogleTranslator(source="auto", target="ko").translate
...
def write_to_page(text, page_number, pdf):
    ...
    pdf.multi_cell(
        ...
        w=0, h=5, txt=text.encode("utf-8", errors="replace").decode("utf-8")
        #w=0, h=5, txt=text.decode('cp949').encode('utf-8')
    )
